...
<!-- add member_id search -->
<form action="./" method="get">
<input type="hidden" name="m" value="({$module_name})" />
<input type="hidden" name="a" value="page_({$hash_tbl->hash('diary_comment_list','page')})" />
<h3 class="item">({$WORD_DIARY})コメントユーザーID検索</h3>
<input class="basic" type="text" name="target_c_member_id" value="({$target_c_member_id})" />
<span class="textBtnS"><input type="submit" value="　検　索　" /></span>
</form>
({$debug})
<!-- member_id search close -->
...
<p class="listMove">
({if $page_list})({foreach from=$page_list item=item})({if $page!=$item})<a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('diary_comment_list','page')})&amp;page=({$item})&amp;keyword=({$keyword_encode})&amp;target_c_diary_id=({$target_c_diary_id})&amp;target_c_member_id=({$target_c_member_id})">({$item})</a>({else})<b>({$item})</b>({/if})&nbsp;&nbsp;({/foreach})&nbsp;({/if})
({if $prev})<a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('diary_comment_list','page')})&amp;page=({$page-1})&amp;keyword=({$keyword_encode})&amp;target_c_diary_id=({$target_c_diary_id})&amp;target_c_member_id=({$target_c_member_id})">＜＜前</a>　({/if})
({if $next})<a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('diary_comment_list','page')})&amp;page=({$page+1})&amp;keyword=({$keyword_encode})&amp;target_c_diary_id=({$target_c_diary_id})&amp;target_c_member_id=({$target_c_member_id})">次＞＞</a>({/if})
</p>
