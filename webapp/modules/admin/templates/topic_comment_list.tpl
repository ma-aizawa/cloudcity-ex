...
<p class="listMove">
({if $page_list})({foreach from=$page_list item=item})({if $page!=$item})<a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('topic_comment_list','page')})&amp;page=({$item})&amp;keyword=({$keyword_encode})&amp;target_c_member_id=({$target_c_member_id})">({$item})</a>({else})<b>({$item})</b>({/if})&nbsp;&nbsp;({/foreach})&nbsp;({/if})
({if $prev})<a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('topic_comment_list','page')})&amp;page=({$page-1})&amp;keyword=({$keyword_encode})&amp;target_c_member_id=({$target_c_member_id})">＜＜前</a>　({/if})
({if $next})<a href="?m=({$module_name})&amp;a=page_({$hash_tbl->hash('topic_comment_list','page')})&amp;page=({$page+1})&amp;keyword=({$keyword_encode})&amp;target_c_member_id=({$target_c_member_id})">次＞＞</a>({/if})
</p>
