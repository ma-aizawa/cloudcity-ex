...
function db_member_c_member_get_freeex_member($c_member_id) {
  $unit_id_of_profile_id = 9;
  $freeex_id_in_profile_option = EX_LINK_CONDITION;

  $sql = 'SELECT c_member_id '
    . '  FROM c_member_profile '
    . '  WHERE c_profile_id = ?'
    . '    AND c_profile_option_id = ?'
    . '    AND c_member_id = ?';

  $params = array(
    $unit_id_of_profile_id,
    $freeex_id_in_profile_option,
    $c_member_id
  );

  $is_freeex_member = false;

  if($freeex_member = db_get_row($sql, $params)) {
    $is_freeex_member = true;
  }

  return $is_freeex_member;
}
