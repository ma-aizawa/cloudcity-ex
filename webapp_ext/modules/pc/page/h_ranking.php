...
class pc_page_h_ranking extends OpenPNE_Action
{
    function execute($requests)
    {
        ...
        switch ($kind) {
        case "point":
            $sql = "SELECT cmp.c_member_id,cmp.value as count FROM c_member_profile as cmp,c_profile as cp" .
                   " WHERE cmp.c_profile_id = cp.c_profile_id AND cp.name = 'PNE_POINT'" .
                   " ORDER BY CAST(cmp.value AS UNSIGNED) DESC";
            $list = db_get_all_limit($sql, 0, $limit, array());
