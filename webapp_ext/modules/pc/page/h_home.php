...
class pc_page_h_home extends OpenPNE_Action
{
    ...
    function execute($requests)
    {
        ...
        // 最新コメント
        if ($OPTION['IS_DISPLAY_NEWCOMMENT_HOME']){
            $this->set('c_diary_comment_list_all', p_h_home_c_diary_new_comment_list(5));
        }
        // フレンド最新日記
        $c_diary_friend_list = p_h_home_c_diary_friend_list4c_member_id($u, 5, true);
        $this->set('c_diary_friend_list', $c_diary_friend_list);
        // フレンド最新blog
        if ($OPTION['IS_DISPLAY_FRIEND_BLOG_HOME']) {
        ...
        // レビュー
        if ($OPTION['IS_DISPLAY_FRIEND_REVIEW_HOME']) {
            $this->set('c_friend_review_list', db_review_c_friend_review_list4c_member_id($u, 5));
        }
        if (OPENPNE_USE_ALBUM && $OPTION['IS_DISPLAY_FRIEND_ALBUM_HOME']) {
            // アルバム
