...
({* {{{ diaryDetailBox *})
  <div class="dparts diaryDetailBox"><div class="parts">
    ...
    ({/if})
    <dl>
    <dt>({$target_diary.r_datetime|date_format:"%Y年<br />%m月%d日<br />%H:%M"})
    ({if $smarty.const.ID_PHOTO_USE})
    <br>
    <img src="({t_img_url filename=$target_member.image_filename w=76 h=76 noimg=no_image})" alt="プロフィール写真" />
    ({/if})
    </dt>
    <dd>
    <div class="title">
...
({* {{{ commentList *})
  <div class="dparts commentList" id="commentList"><div class="parts">
    ...
    <dl>
    <dt>({$item.r_datetime|date_format:"%Y年<br />%m月%d日<br />%H:%M"})({if $type == "h"})<br /><input type="checkbox" class="input_checkbox" name="target_c_diary_comment_id[]" value="({$item.c_diary_comment_id})" />({/if})
    ({if $smarty.const.ID_PHOTO_USE})
    <br>
    <img src="({t_img_url filename=$item.img w=76 h=76 noimg=no_image})" alt="プロフィール写真" />
    ({/if})
    </dt>
    <dd>
    <div class="title">
...
({* {{{ formTable *})
  <div class="dparts formTable" id="commentForm"><div class="parts">
    ...
    <input type="hidden" name="target_c_diary_id" value="({$target_diary.c_diary_id})" />
    <table><tr>
    <th>本文</th><td><textarea name="body" id="comment_box" rows="8" cols="40">({if $requests.body})({$requests.body})({else})イイネ☆({/if})</textarea></td>
    </tr><tr>
