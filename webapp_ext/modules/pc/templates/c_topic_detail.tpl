...
({* {{{ topicDetailBox *})
        <div class="dparts topicDetailBox"><div class="parts">
                ...
                <dl>
                <dt>({$c_topic.r_datetime|date_format:"%Y年%m月%d日"})<br />({$c_topic.r_datetime|date_format:"%H:%M"})
({if $smarty.const.ID_PHOTO_USE})
<br>
<img src="({t_img_url filename=$c_topic.image_filename w=76 h=76 noimg=no_image})" alt="プロフィール写真" />
({/if})
                </dt>
                <dd>
                <div class="title">
                ...
                ({if $is_made || $is_c_commu_admin || ($is_c_topic_admin && $c_commu.is_topic !== 'admin_only')})
                ({if $is_made || $is_c_commu_member || $c_commu.is_topic == 'public'})
                <div class="operation">
...
({* {{{ commentList *})
        <div class="dparts commentList"><div class="parts">
                ...
                <dl>
                <dt>({$item.r_datetime|date_format:"%Y年<br />%m月%d日<br />%H:%M"})
                ({if $smarty.const.ID_PHOTO_USE})
                <br>
<img src="({t_img_url filename=$item.image_filename w=76 h=76 noimg=no_image})" alt="プロフィール写真" />
                ({/if})
                </dt>
                <dd>
                <div class="title">
